...
export const metadata = {
  title: "Blog - John Doe",
  description: "Read my latest articles on software development, design, and more.",
}
  ...
  return (
    <main className="container py-12 md:py-24 lg:py-32">
      <div className="flex flex-col items-center justify-center space-y-4 text-center">
        <div className="space-y-2">
          ...
          <p className="max-w-[900px] text-muted-foreground md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed">
            Thoughts, ideas, and insights on software development, design, and more.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 justify-center mt-8">
        <Link href="/blog">
          <Button variant="outline" size="sm" className={!categorySlug ? "bg-primary text-primary-foreground" : ""}>
            All
          </Button>
        </Link>
        {categories &&
          ...
          categories.map((cat) => (
            <Link key={cat.id} href={`/blog?category=${cat.slug}`}>
              <Button
                variant="outline"
                size="sm"
                className={categorySlug === cat.slug ? "bg-primary text-primary-foreground" : ""}
              >
                {cat.title}
              </Button>
            </Link>
